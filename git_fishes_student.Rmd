...
```{r}
## Omitting any rows that have NA data. 
data_clean <- na.omit(data)
data_clean
```

```{r}
## Dropping irrelevant columns (comment and loc)
data_cleaner <- subset(data_clean, select = -c(comment, loc))
data_cleaner
```

```{r}
## Checking that there are no spelling issues in species and treatment and create a table of summary data that includes:

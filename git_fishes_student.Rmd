...
[My GitHub Repository](https://github.com/u7117486/hayleyrepo)
...
## Because it won't keep track of your entire code in a good consistent format, it will just keep track of changes etc, when they are committed to the reposiotry. 
...
## If the echo=TRUE is changed to FALSE, it will hide the whole code chunk from the render. 
```
...
data_cleaner <- subset(data_clean, select = -c(comment, loc))
# Check spelling in species and treatment but also generate a summary table
summary <- data_cleaner %>% group_by(species, treatment) %>% summarise(mean(activity), sd(activity), n())
...
## Red is deleted text, and green is new text. 
```
...
ggplot(data_cleaner, aes(x=species, y=activity, colour=treatment))+
  geom_boxplot()+
  theme_bw()
```
...
ggsave('output/figures/species_v_activity', device = png,  width = 6, height = 4)
```
...
```{r}
## I see the file path that I wrote in the text file, nothing else happened. 
```
...
chromis_lemon_acantho <- data_cleaner[which(data_cleaner$species=="chromis"|data_cleaner$species=="lemon"|data_cleaner$species=="acantho"),]
ggplot(chromis_lemon_acantho, aes(x=species, y=activity, colour=treatment))+
  geom_boxplot()+
  theme_bw()
```
...
```{r}
## Task 7.5 - it said there was a conflicted file

## Task 7.7 - It has mashed together our two commits into the file. 
